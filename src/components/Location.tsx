...
const highlights = [
{ icon: LandmarkIcon, title: 'Sightseeing', text: 'Kanini sits close to the landmarks visitors come to Ndola for.' },
{ icon: TreePalmIcon, title: 'Recreation', text: 'Parks, sport and places to eat are all within a short drive.' },
{ icon: RouteIcon, title: 'Getting around', text: 'Broadway keeps the city centre and main routes easy to reach.' }];
  ...
  return (
    <section id="location" className="w-full bg-ink py-28 text-cream lg:py-40">
      <div className="mx-auto grid max-w-[1240px] grid-cols-1 items-center gap-16 px-6 lg:grid-cols-2 lg:gap-24 lg:px-10">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-overline text-gold-light">Location</p>
          ...
          <p className="mt-7 max-w-md text-[15px] font-light leading-[1.85] text-cream/70">
            The neighbourhood scores 4.3 with visitors for sightseeing, recreation and ease of getting around &mdash;
            which makes Broadway a practical base whether you are here for two nights or two weeks.
          </p>

          <ul className="mt-14 space-y-9">
            {highlights.map(({ icon: Icon, title, text }) =>
            <li key={title} className="flex gap-5 border-t border-white/10 pt-7">
                <Icon className="mt-0.5 h-5 w-5 shrink-0 text-gold-light" strokeWidth={1.5} aria-hidden="true" />
                <div>
                  <h3 className="text-[15px] font-semibold">{title}</h3>
                  <p className="mt-1.5 text-[14px] font-light leading-relaxed text-cream/65">{text}</p>
                </div>
              </li>
            )}
          </ul>
